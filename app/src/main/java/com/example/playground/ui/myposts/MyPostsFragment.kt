...
class MyPostsFragment : Fragment() {
    ...
    private fun showEditDialog(event: Event) {
        parentFragmentManager.beginTransaction()
            .replace(R.id.nav_host_fragment, EditEventFragment.newInstance(event.id))
            .addToBackStack(null)
            .commit()
    }
    // ---- Adapter ----
    class MyEventsAdapter(
        ...
        private fun formatTimeAgo(timestamp: Long): String {
            val diff = timestamp - System.currentTimeMillis() // future = positive
            val minutes = diff / 60_000
            return when {
                diff < 0 -> "Started ${-diff / 60_000}m ago"  // past
